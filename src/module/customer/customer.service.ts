...
export class CustomerService {
  ...
  async remove(id: string, user: UserDocument): Promise<void> {
    ...
    try {
      const result = await this.customerModel.findOneAndDelete({ _id: id, user: user._id }).exec();
      if (!result) {
        throw new NotFoundException(`Customer with ID ${id} not found`);
      }
      this.logger.log(`Deleted customer id=${id}`);
    } catch (error) {
      this.logger.error(`Failed to delete customer id=${id}: ${error.message}`, error.stack);
      throw error instanceof NotFoundException
        ? error
        : new InternalServerErrorException(`Failed to delete customer with ID ${id}`);
    }
  }
