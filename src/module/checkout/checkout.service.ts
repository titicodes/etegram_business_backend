...
//   async checkProduct(code: string, storeId: string, userId: string, userRole: UserRoleEnum[]): Promise<ProductDocument> {
...
//       throw new NotFoundException(`Product with barcode ${code} not found in store ${storeId}`);
//     }
...
//     this.logger.log(`Product checked successfully: Code=${code}, Product=${product.name}`);
//     return product;
//   }
...
//         const product = await this.productModel.findOne({ code, store: new Types.ObjectId(storeId) }).exec();
//         if (product) {
//           await this.firebaseService.updateProductStock(product);
//         }
//       }
...
//       await this.firebaseService.trackOrderStatus(checkout._id.toString(), isCredit ? 'Pending' : 'Completed');

//       if (user.fcmToken) {
//         await this.firebaseService.sendNotification(
//           user.fcmToken,
//           isCredit ? 'Credit Sale Recorded' : 'Order Confirmed',
//           `Your ${isCredit ? 'credit' : 'order'} has been ${isCredit ? 'recorded' : 'placed'} successfully! Total: ₦${totalPriceWithTax}`,
//           { orderId: checkout._id.toString() },
//         );
//       }

//       await this.notificationService.sendOrderNotification(user._id.toString(), checkout);
...
//   async getSalesHistory(storeId: string, userId: string, userRole: UserRoleEnum[], productId?: string): Promise<CheckoutDocument[]> {
//     this.logger.log(`Fetching sales history: Store=${storeId}, User=${userId}, Role=${userRole}, ProductId=${productId || 'all'}`);
...
//     const sales = await this.checkoutModel.find(query).sort({ createdAt: -1 }).exec();
//     this.logger.log(`Fetched ${sales.length} sales records for store ${storeId}`);
//     return sales;
//   }

//   async getOwingRecords(storeId: string, userId: string, userRole: UserRoleEnum[], supplierId?: string): Promise<CheckoutDocument[]> {
//     this.logger.log(`Fetching owing records: Store=${storeId}, User=${userId}, Role=${userRole}, SupplierId=${supplierId || 'all'}`);
...
//     const query: any = { store: new Types.ObjectId(storeId), supplierId: { $exists: true } };
//     if (supplierId && Types.ObjectId.isValid(supplierId)) {
//       query.supplierId = new Types.ObjectId(supplierId);
//     }

//     const owing = await this.checkoutModel.find(query).sort({ createdAt: -1 }).exec();
//     this.logger.log(`Fetched ${owing.length} owing records for store ${storeId}`);
//     return owing;
//   }

//   async getOwedRecords(storeId: string, userId: string, userRole: UserRoleEnum[], customerId?: string): Promise<CheckoutDocument[]> {
//     this.logger.log(`Fetching owed records: Store=${storeId}, User=${userId}, Role=${userRole}, CustomerId=${customerId || 'all'}`);
...
//     const owed = await this.checkoutModel.find(query).sort({ createdAt: -1 }).exec();
...
//     await this.notificationService.sendOrderStatusUpdate(orderId, userId, status);
...
//           throw new BadRequestException(`Product with code ${code} not found in store ${storeId}`);
//         }
...
//     this.logger.debug(`Store access check: Store=${storeId}, User=${userId}, Role=${userRole}, Found=${!!store}`);
//     return store;
//   }
...
export class CheckoutService {
  ...
  async createCheckout(createCheckoutDto: CreateCheckoutDto, user: UserDocument): Promise<{ checkout: CheckoutDocument; emailWarning?: string }> {
    const { cart, discount = 0, tax = 0, paymentMethod, storeId, isCredit = false, customerId, supplierId, deliveryAddress } = createCheckoutDto;
    ...
    if (discount > 0 && (user.role.includes(UserRoleEnum.STORE_OWNER) || user.role.includes(UserRoleEnum.ADMIN))) {
      appliedDiscount = discount;
      this.logger.log(`Applying discount of ${discount}% by user ${user._id}`);
    } else if (discount > 0) {
      this.logger.warn(`User ${user._id} attempted to apply discount without permission`);
      throw new UnauthorizedException('Only store owners or admins can apply discounts');
    }
    ...
    try {
      ...
      const orderData = {
        orderId: checkout._id.toString(),
        customerName: checkout.customerName || `${user.firstName} ${user.lastName}`,
      ...
      try {
        const invoicePath = await this.invoiceService.generateInvoice(orderData);
        await this.emailService.sendInvoice(user.email, invoicePath, orderData);
      } catch (error) {
        emailWarning = 'Invoice email could not be sent due to a server issue. Please check your email later.';
