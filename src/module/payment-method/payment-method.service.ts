...
export class PaymentMethodService {
  ...
  async create(
    ...
    this.logger.log(
      `Creating payment method for user=${user._id}, store=${createDto.storeId}`,
    );
  ...
  async findAll(
    ...
    this.logger.log(
      `Fetching payment methods for user=${user._id}, store=${storeId || 'all'}`,
    );
  ...
  async findOne(id: string, user: UserDocument): Promise<PaymentMethod> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch payment method id=${id}: ${error.message}`,
        error.stack,
      );
      throw error instanceof NotFoundException
        ? error
        : new InternalServerErrorException(
            `Failed to fetch payment method with ID ${id}`,
          );
    }
  }
  ...
  async update(
    ...
    if (
      updateDto.storeId &&
      updateDto.storeId !== paymentMethod.store.toString()
    ) {
      const store = await this.validateStoreAccess(
        updateDto.storeId,
        user._id.toString(),
    ...
    try {
      ...
      this.logger.error(
        `Failed to update payment method id=${id}: ${error.message}`,
        error.stack,
      );
      throw error instanceof NotFoundException
        ? error
  ...
  async remove(id: string, user: UserDocument): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to delete payment method id=${id}: ${error.message}`,
        error.stack,
      );
      throw error instanceof NotFoundException
        ? error
