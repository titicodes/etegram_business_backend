...
import { Subscription, SubscriptionDocument } from '../subscription/schema/subscription.schema';
...
export class AdminController {
    ...
    async createUser(@Body() dto: CreateUserDto, @Request() req: { user: User }): Promise<User> {
    ...
    async getUserById(@Param('id') id: string, @Request() req: { user: User }): Promise<User> {
    ...
    @ApiResponse({ status: 404, description: 'User not found' })
    async updateUser(@Param('id') id: string, @Body() updateDto: Partial<CreateUserDto>, @Request() req: { user: User }): Promise<User> {
    ...
    async createStore(@Body() dto: CreateStoreDto, @Body('ownerId') ownerId: string, @Request() req: { user: User }): Promise<Store> {
        return this.adminService.createStore(dto, ownerId, req.user);
    }

    @Post('stores/branch')
    @ApiOperation({ summary: 'Create a store branch' })
    @ApiResponse({ status: 201, description: 'Branch created successfully', type: Store })
    @ApiResponse({ status: 400, description: 'Invalid input' })
    ...
    async createStoreBranch(
        ...
        return this.adminService.createStoreBranch(dto, ownerId, parentStoreId, req.user);
    }

    @Get('stores/owner/:ownerId')
    @ApiOperation({ summary: 'Get stores by owner' })
    @ApiResponse({ status: 200, description: 'Stores retrieved successfully', type: [Store] })
    @ApiResponse({ status: 400, description: 'Invalid owner ID' })
    async getStoresByOwner(@Param('ownerId') ownerId: string, @Request() req: { user: User }): Promise<Store[]> {
    ...
    async getStoreById(@Param('id') id: string, @Request() req: { user: User }): Promise<Store> {
    ...
    @ApiResponse({ status: 404, description: 'Store not found' })
    async updateStore(@Param('id') id: string, @Body() dto: UpdateStoreDto, @Request() req: { user: User }): Promise<Store> {
    ...
    @ApiResponse({ status: 400, description: 'Invalid store ID' })
    @ApiResponse({ status: 404, description: 'Store not found' })
    async deleteStore(@Param('id') id: string, @Request() req: { user: User }): Promise<void> {
    ...
    async getProductById(@Param('id') id: string, @Param('storeId') storeId: string, @Request() req: { user: User }): Promise<ProductDocument> {
    ...
    @ApiResponse({ status: 401, description: 'Unauthorized access' })
    @ApiResponse({ status: 404, description: 'Product not found' })
    async deleteProduct(@Param('id') id: string, @Body('storeId') storeId: string, @Request() req: { user: User }): Promise<void> {
    ...
    @ApiResponse({ status: 400, description: 'Invalid store ID or filter parameters' })
    async getProductsFiltered(
        ...
        return this.adminService.getProductsFiltered(storeId, { ...filter, page, limit }, req.user);
    }
    ...
    @ApiResponse({ status: 400, description: 'Invalid user ID' })
    @ApiResponse({ status: 404, description: 'User not found' })
    async createSubscription(@Body('userId') userId: string, @Request() req: { user: User }): Promise<SubscriptionDocument> {
    ...
    @ApiResponse({ status: 200, description: 'User subscribed to premium successfully', type: Subscription })
    @ApiResponse({ status: 400, description: 'Invalid user ID or subscription type' })
    ...
    async subscribeToPremium(
        ...
        return this.adminService.subscribeToPremium(userId, subscriptionType, req.user);
    }

    @Get('subscriptions/:userId')
    @ApiOperation({ summary: 'Get subscription status' })
    @ApiResponse({ status: 200, description: 'Subscription status retrieved successfully', type: Subscription })
    @ApiResponse({ status: 400, description: 'Invalid user ID' })
    @ApiResponse({ status: 404, description: 'Subscription not found' })
    async getSubscriptionStatus(@Param('userId') userId: string, @Request() req: { user: User }): Promise<SubscriptionDocument> {
        return this.adminService.getSubscriptionStatus(userId, req.user);
    }

    @Delete('subscriptions/:userId')
    @ApiOperation({ summary: 'Cancel subscription' })
    @ApiResponse({ status: 200, description: 'Subscription canceled successfully', type: Subscription })
    @ApiResponse({ status: 400, description: 'Invalid user ID' })
    @ApiResponse({ status: 404, description: 'Subscription not found' })
    async cancelSubscription(@Param('userId') userId: string, @Request() req: { user: User }): Promise<SubscriptionDocument> {
        return this.adminService.cancelSubscription(userId, req.user);
    }

    @Get('checkouts/sales')
    @ApiOperation({ summary: 'Get sales history' })
    @ApiResponse({ status: 200, description: 'Sales history retrieved successfully', type: [Checkout] })
    ...
    @ApiResponse({ status: 200, description: 'Owing records retrieved successfully', type: [Checkout] })
    ...
    @ApiResponse({ status: 200, description: 'Owed records retrieved successfully', type: [Checkout] })
    ...
    @ApiResponse({ status: 200, description: 'Order status updated successfully', type: Checkout })
