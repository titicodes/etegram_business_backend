...
export class ProductService {
   ...
   */
  async supplyProduct(createProductDto: CreateProductDto): Promise<Product> {
    const { name, categoryId, price, stock, unitPrice, expiryDate, supplyTo } = createProductDto;

    // Ensure the category exists
    const existingCategory = await this.categoryService.findOne(categoryId);
    if (!existingCategory) throw new NotFoundException('Category not found');

    // Check if product exists by name (since we are not searching by code)
    const existingProduct = await this.productModel.findOne({ name });

    if (existingProduct) {
      // Update stock and allow optional edits
      existingProduct.stock += stock;
      if (price) existingProduct.price = price;
      if (unitPrice) existingProduct.unitPrice = unitPrice;
      if (expiryDate) existingProduct.expiryDate = expiryDate;
      if (supplyTo) existingProduct.supplyTo = supplyTo;

      return existingProduct.save();
    } else {
      // Require scanning for a new product
      throw new NotFoundException('Product not found. Please scan to add a new product.');
    }
  }
