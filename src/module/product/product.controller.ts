...
export class ProductController {
  ...
  async addProduct(
    ...
    const parsedMinQuantity =
      minQuantity !== undefined ? parseInt(minQuantity, 10) : undefined;
  ...
  async checkProductExistence(
    ...
    const result = await this.productService.checkProductExistenceByCode(
      code,
      req.user._id,
      storeId,
    );
  ...
  async sendOutProduct(
    ...
    return this.productService.sendOutProduct(
      dto,
      req.user._id,
      storeId,
      req.user.role,
  ...
  async receiveProduct(
    ...
    return this.productService.receiveProduct(
      dto,
      req.user._id,
      storeId,
      req.user.role,
  ...
  async getExpiringProducts(
    ...
    this.logger.log(
      '[ProductController][getExpiringProducts] Request received:',
      { storeId, query, user: req.user },
    );
  ...
  async findOne(
    ...
    this.logger.log('[ProductController][findOne] Request received:', {
      id,
      storeId,
    });
  ...
  async updateProduct(
    ...
    let finalImageUrl = imageUrl;
    if (finalImageUrl === '' || finalImageUrl === 'null') {
      // Handle both empty string and the string 'null'
      finalImageUrl = undefined;
    }
  ...
  async uploadProductImage(
    ...
    if (!file && !imageUrl) {
      // Allow clearing by sending no file and no URL or explicit null
      imageUrl = null; // Normalize to null for service logic if both are empty/undefined
    } else if (imageUrl === '') {
      // Treat empty string URL as desire to clear
      imageUrl = null;
    }
    ...
    if (!file && imageUrl === null) {
      ...
      throw new BadRequestException(
        'Either an image file or URL must be provided to set an image',
      );
    }
  ...
  async deleteProduct(
    ...
    if (!storeId) {
      this.logger.error(
        '[ProductController][deleteProduct] Missing storeId in request body',
      );
