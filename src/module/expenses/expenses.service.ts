...
export class ExpenseService {
    ...
    async createExpense(createDto: CreateExpenseDto, user: UserDocument): Promise<Expense> {
        this.logger.log(`Creating expense for user=${user._id}, store=${createDto.storeId}`);
    ...
    async findExpenseById(id: string, user: UserDocument): Promise<Expense> {
        ...
        try {
            ...
            this.logger.error(`Failed to fetch expense id=${id}: ${error.message}`, error.stack);
    ...
    async updateExpense(id: string, updateDto: UpdateExpenseDto, user: UserDocument): Promise<Expense> {
        ...
        try {
            ...
            this.logger.error(`Failed to update expense id=${id}: ${error.message}`, error.stack);
            throw error instanceof NotFoundException
                ? error
    ...
    async deleteExpense(id: string, user: UserDocument): Promise<{ deleted: boolean }> {
        ...
        try {
            ...
            this.logger.error(`Failed to delete expense id=${id}: ${error.message}`, error.stack);
            throw error instanceof NotFoundException
                ? error
