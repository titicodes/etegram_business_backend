...
export class SubscriptionService {
  ...
  async createSubscription(userId: string): Promise<SubscriptionDocument> {
    ...
    if (user.fcmToken) {
      ...
      this.logger.warn(
        `User ${userId} has no FCM token. Skipping trial start notification.`,
      );
    }
  ...
  async subscribeToPremium(
    ...
    if (!user || !user.fcmToken) {
      this.logger.warn(
        `User ${userId} has no FCM token. Skipping premium subscription notification.`,
      );
    } else {
      await this.notificationService.sendPremiumSubscriptionNotification(
        user.fcmToken,
        subscriptionType,
      );
    }
    ...
    endDate.setDate(
      currentDate.getDate() + (subscriptionType === 'MONTHLY' ? 30 : 365),
    );
  ...
  async checkTrialExpirations(): Promise<void> {
    ...
    for (const subscription of subscriptions) {
      ...
      if (fcmToken) {
        await this.notificationService.sendTrialExpirationNotification(
          fcmToken,
        );
      } else {
        this.logger.warn(
          `User ${userId} has no FCM token. Skipping trial expiration notification.`,
        );
      }
    }
  }
  ...
  async sendTrialReminder(): Promise<void> {
    ...
    for (const subscription of subscriptions) {
      ...
      if (fcmToken) {
        ...
        this.logger.warn(
          `User ${userId} has no FCM token. Skipping trial reminder notification.`,
        );
      }
    }
  }
  ...
  async cancelSubscription(userId: string): Promise<SubscriptionDocument> {
    ...
    if (!user || !user.fcmToken) {
      this.logger.warn(
        `User ${userId} has no FCM token. Skipping subscription cancellation notification.`,
      );
    } else {
      await this.notificationService.sendSubscriptionCancellationNotification(
        user.fcmToken,
      );
    }
